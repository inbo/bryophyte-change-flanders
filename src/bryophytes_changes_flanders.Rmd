...
full_model_zib <- brm(bf(n_hok_2000_2019 | trials(n_hok_tot_2000_2019)
                         + weights(wei) ~ 
                  ...
                  + ell_t:phylum
                  + (1 + phylum || substrate),
                  zi ~   qlogis((n_hok_1980_1999+0.5)/(n_hok_tot_1980_1999+1))
                  + ell_l
                  + ell_n
                  + ell_f
                  + ell_t
...
loo1$diffs %>%
  as.data.frame() %>%
  kable(caption = "Results of model comparisons. First row contains the model with the highest predictive accuracy.",
        digits = 2)
```
...
broom.mixed::tidy(model_zib_2) %>%
  kable(
    digits = 2,
    caption = "Parameter estimates for the zero-inflated (component = zi) and binomial part (component = cond) of the final model.")
```
